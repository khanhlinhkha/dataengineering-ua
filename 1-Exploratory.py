...
#Here we are looking at number of arrests per beat (more specific than district), we wanted to identify a threshold for which beats are most important
...
for district in district_primary_counts.index:
    ...
    # Check if there are any Primary Types above the threshold; districts that don't have any primary type counts above the threshold are neglected
...
# This could give you more information about the location. When arrested, which crimes were caught the most per district. Also about the distribution per district.
# Some districts are highly concentrated on a couple of primary types for example.
# That way, you can identify which districts are most 'responsible' for NARCOTICS, THEFT, BATTERY, which were the three most common primary types

#We saw that narcotics has highest count so we will look further into this
...
# We are interested WHEN the police arrests someone (in general first), afterwards specific for narcotics. (We can also dive deeper on Battery and Theft)
